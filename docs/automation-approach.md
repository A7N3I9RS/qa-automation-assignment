...
Cart and checkout specs run their functional shopper scenarios against those users. Checkout visual tests also run for each shopping user, comparing them against the `standard_user` baseline so user-specific visual differences appear in the report.
...
Visual snapshots use the project name in their path, so Chromium, Firefox and WebKit keep separate baselines and never compare screenshots across browsers. The checkout visual baseline is captured with `standard_user`; other shopping users compare against that same browser-specific baseline.
